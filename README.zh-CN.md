...
|  | 没有它 | 有了这套框架 |
|---|---|---|
| 🔧 **改个错别字** | AI 把整本 14KB 宪章加你全部偏好塞进上下文 | 判为 **QUICK**,只读 3 个小文件(约 5500 字)就动手 |
| 💻 **换台电脑** | 「项目在哪?谁能提交?哪份是正本?」 | 方法一字不改,你只要重填这台机的事实 |
| 💬 **多个对话同时开** | 各说各话、互相覆盖、出事说不清 | 一套权威顺序加四维状态加门禁,每处冲突都能追溯 |
| 📋 **一份进度汇报** | 一大段「我理解了……」,毫无证据 | 真改了文件才给三行:**Changed / Verified / Remaining**;纯聊天不套模板 |
...
- **规则能追回宪章。** 16 条 `GOV-*` 规则在 `migration/rule-traceability.yaml` 里逐条钉到出处,标 `none`(原文抽取)或 `added`(新增),你可以核对 runtime 层有没有偏离它所概括的宪章。
...
完整功能目录见 **[FEATURES.md](FEATURES.md)** —— 全清单,连 README 装不下的都在里面。
...
clone 下来，直接跟你的 AI 说:**“读 `BOOTSTRAP.md`，帮我把这套框架配好。”** 它会读宪章、给你一张短清单（你的角色、你有哪些项目、在哪、谁能提交、装哪些红线），按你的机器适配模板，再告诉你配好了什么、还差你做什么。
...
> 2. 只加载 runtime 这一层:入口、16 条承重规则、你的画像(约 5500 字),跳过整本 14KB 宪章和所有手册,因为存在不等于每次都要读。
> 3. 改好、重读那行确认,给你三行回执:
>
> ```text
...
框架还带第二个 skill:[`growth-secretary`](skills/growth-secretary/SKILL.md)。它按周或月(或你点名时)复盘你最近的真实工作——先取硬数字:逐仓 git 提交、会话统计、交付物实地清点——把一段时间整理成三种交付:内部复盘、可对外的工作汇报(纯事实陈述,不带修辞腔)、可视化面板。它列出哪些偏好值得记,跟你确认,只有你点头才把稳定的习惯写进画像。它会随着使用越来越贴合你,但不会背着你记东西。
...
框架会自我纠正:发现自己跑偏就停下来改回去(`GOV-013`);不管文档是什么语言,都用你的语言回复(`GOV-015`);被问到不归它管的事,简单答一句再给你指个方向,不越权(`GOV-014`);动手做任何东西前,先找现有方案,别从零造(`GOV-016`)。
