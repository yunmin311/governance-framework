...
## 基准线（2026-08-14 用户定稿，照此做、别另起炉灶）

一份「全量统计」页在真实复盘中跑通并被用户定为本层基准线。要素：

- **底**：青瓷蓝纸底 + 卡纸纹理叠底（设计库 `public/assets/textures/paper_q90.jpg` 之类的纸纹，base64 内嵌、`mix-blend-mode:multiply`、透明度约 .5，单文件自包含）；
- **版式**：数字墙（大数 Georgia serif、细分说明小字）→ 总量趋势面积图 → 逐日热力图（GitHub 式日历格，单色蓝阶按量分档、格子阶梯弹入、附图例与阈值）→ 小倍数条形（每实体一格、独立刻度、避免多色分类）→ 横向分布条 → 里程碑时间轴 → 实体卡片 → 键值面板 → 文字结论；附可展开数据表；
