...
> 设计原则:正本永远是 git+markdown,所有机制靠 frontmatter + 约定实现,**不上任何数据库/向量库/常驻服务**;跨模型可移植(纯文本,各 agent 都能读写);把系统已有的 **GOV 事实优先级 / 一事一正本 / 证据纪律**延伸进记忆层,不推翻四层。完整设计与生命周期演算留在维护者自己的私有笔记里,不随框架发布。
