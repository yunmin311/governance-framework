...
1. `tools\usage_stats.mjs` + `render_usage.mjs` 出一次用量看板(不是"可选加分项",是复盘的取证之一);
2. 把这段冒出的稳定事实**追加成 delta** 写进记忆(不整篇重写、不拿单次当习惯、改画像先问);
3. 跑 `<治理仓>\tools\sync_memory.ps1 -Push` 把记忆同步进仓(SessionEnd 钩子会自动做,但**手动确认一次**——钩子只在主库有变化时才触发,别的工作目录的记忆变了它不一定醒)。
